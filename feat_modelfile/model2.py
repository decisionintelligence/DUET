...
class QuantumOTOCBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 修改测试点1
        # 利用otoc权重矩阵，计算李雅普诺夫指数，作为损失函数的一部分
...
class AdaptiveFusion(nn.Module):
    ...
    def forward(self, quantum_feat: torch.Tensor, transformer_feat: torch.Tensor) -> torch.Tensor:
        ...
        # 修改测试点4
        # 此处权重融合的初始值可能有很大影响，是否考虑给定相同初始值，再进行训练，观察权重变化趋势，剔除不必要的融合方法？或者引入温度参数控制权重分布的平滑程度？
...
class DUETModel(nn.Module):
    def __init__(self, config):
        ...
        # 修改测试点2
        # Channel_transformer能否加入AttentionResiduals进行优化？
