...
class QuantumOTOCBlock(nn.Module):
    """
    Quantum-inspired feature mixing block - v6 优化版本

    优化点：
    1. OTOC损失函数：李雅普诺夫指数的近似计算
    2. 更稳定的归一化
    3. 特征调制增强
    """
    ...
    def __init__(self, d_model: int, n_heads: int = 4):
        ...
        self.se_fc3 = nn.Linear(d_model, d_model)  # 新增：特征调制
    ...
    def _se_gate(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # 特征调制
        mod = torch.tanh(self.se_fc3(s))

        return s.unsqueeze(1), mod.unsqueeze(1)

    def _compute_lyapunov_approximation(self, prob_matrix: torch.Tensor) -> torch.Tensor:
        """
        优化1：计算OTOC的近似李雅普诺夫指数

        OTOC(t) ∝ exp(λ * t)，其中λ是李雅普诺夫指数
        我们用时间步间的OTOC变化来近似计算
        """
        # 避免log(0)
        prob_safe = prob_matrix.clamp(min=1e-8, max=1-1e-8)

        # 计算香农熵 (退相干的度量)
        entropy = -torch.sum(prob_safe * torch.log(prob_safe), dim=-1)

        # 归一化
        entropy = entropy / (entropy.mean(dim=-1, keepdim=True) + 1e-6)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        se_weight, se_mod = self._se_gate(x)
        ...
        # 优化2：更稳定的归一化
        psi_norm = psi_0.abs().sum(dim=-1, keepdim=True).clamp_min(1e-6)
        psi_0 = psi_0 / psi_norm
        ...
        # 优化3：使用李雅普诺夫近似计算权重
        lyapunov_weight = self._compute_lyapunov_approximation(prob_matrix)
        otoc_weight = F.softmax(lyapunov_weight, dim=-1)
        ...
        z_out = z_out + se_mod * x_residual  # 残差特征调制
